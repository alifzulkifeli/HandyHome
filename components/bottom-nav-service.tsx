...
const BottomNavService = ({serviceroviderID}:Props) => {
				...
				<div className=' flex h-14  items-center justify-around'>
					{links.map(({ href, label, icon , cname},index) => (
						<Link
							...
							href={index === 0 ? (href + "/" + serviceroviderID) : {href}}
							className={`flex h-full w-full flex items-center justify-center space-y-1 ${cname}`}
						>
							{icon}
